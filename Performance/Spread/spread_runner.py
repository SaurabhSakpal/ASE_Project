...
def spread_calculator_wrapper(dr):
    global model_name
    ...
    for algorithm_name, content in zip(algorithm_names, contents):
        ...
        result_dic[algorithm_name] = round(spread,3)

    return result_dic

def writeToStatFile(spreads):
    global OUT_PATH
    global model_name
    if not os.path.exists(OUT_PATH):
        os.makedirs(OUT_PATH)
    f = open(OUT_PATH+'spread_'+model_name,'w')
    for opt in spreads:
        f.write(opt+' '+' '.join(map(str,spreads[opt]))+'\n')
...
for dr in os.listdir(IN_PATH):
    rslt = spread_calculator_wrapper(IN_PATH+dr)
    for algo in rslt:
